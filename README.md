...
Скрипт сам выбирает установленный Python в порядке приоритета: **3.12 → 3.11 → 3.10** — никаких параметров указывать не требуется. Если подходящая версия не найдена, отобразится подсказка об установке.
