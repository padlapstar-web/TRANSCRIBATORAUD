...
Bootstrap-скрипты создают виртуальное окружение `.venv`, устанавливают зависимости и выполняют smoke-сборку PyInstaller.
FFmpeg при этом ожидается в системе (через `PATH`, `FFMPEG_HOME` или `FFMPEG_BIN`) — пост-обработка сборки
скопирует бинарники в `dist/`, не затрагивая git.
...
При первом запуске приложение проверяет наличие FFmpeg через `PATH`, `FFMPEG_HOME` и каталоги рядом с
приложением. Результат отображается в логе окна; загрузку необходимо выполнять вручную (например,
через пакетный менеджер или официальный сайт ffmpeg.org).
...
FFmpeg и ffprobe нужны для проверки и конвертации аудио, но бинарники запрещено хранить в git. Установи FFmpeg любым удобным
способом (Chocolatey, winget, официальный архив) и убедись, что `ffmpeg.exe` и `ffprobe.exe` доступны через `PATH`,
`FFMPEG_HOME` или `FFMPEG_BIN`. Проверить доступность можно командой `TRANSCRIBATORAUD.exe --check-ffmpeg` (или
`python app/cli.py --check-ffmpeg`). Сборочные скрипты при необходимости скопируют найденные бинарники в `dist/`, а в репозитории
останется только `resources/ffmpeg/.gitkeep`.
...
- `--check-ffmpeg` — быстро проверить, видит ли приложение установленный FFmpeg.
